...
- `list-tables.service.ts`, `get-current-table-session.service.ts`, `list-branch-ready-summary.service.ts` (módulo `orders`) y `set-table-zone.service.ts` (`PATCH /floor/tables/:tableId/zone`, ver doc 20): las cuatro rutas que exponen `assignedStaffUserId` consultan `BranchSettings.tableAssignmentEnabled` y devuelven `null` en vez del valor real de la base si la sucursal tiene la función apagada. Las primeras tres son de lectura; `set-table-zone.service.ts` es un endpoint de escritura de zonas que igual devuelve el campo enmascarado por consistencia con el resto. Esto se agregó después de la revisión: sin este chequeo, una sucursal que activó la función, acumuló asignaciones, y después la desactivó, seguía exponiendo asignaciones viejas por las rutas de lectura — la función no era realmente inerte al apagarla, solo dejaba de aceptar escrituras nuevas.
