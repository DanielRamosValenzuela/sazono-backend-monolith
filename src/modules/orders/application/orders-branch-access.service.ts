...
export type OrdersBranchAccessContext = {
  staffUserId: string;
  restaurantId: string;
  branchId: string;
  roles: Role[];
...
@Injectable()
export class OrdersBranchAccessService {
  ...
  async ensureAccess(
    ...
    branchId: string,
    allowedRoles: Role[],
  ): Promise<OrdersBranchAccessContext> {
    if (
      authUser.profileType !== LoginProfileType.STAFF ||
      !authUser.restaurantId
    ) {
      throw new ForbiddenException(
        'Solo un staff autenticado puede operar sobre el modulo orders.',
      );
    }
