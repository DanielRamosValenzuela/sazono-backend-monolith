...
export type AnalyticsBranchAccessContext = {
  staffUserId: string;
  restaurantId: string;
  branchId: string;
  roles: Role[];
...
@Injectable()
export class AnalyticsBranchAccessService {
  ...
  async ensureAccess(
    ...
    branchId: string,
    allowedRoles: Role[],
  ): Promise<AnalyticsBranchAccessContext> {
    if (
      authUser.profileType !== LoginProfileType.STAFF ||
      !authUser.restaurantId
    ) {
      throw new ForbiddenException(
        'Solo un staff autenticado puede operar sobre el modulo analytics.',
      );
    }
