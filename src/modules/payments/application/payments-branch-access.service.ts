...
export type PaymentsBranchAccessContext = {
  staffUserId: string;
  restaurantId: string;
  branchId: string;
  roles: Role[];
...
@Injectable()
export class PaymentsBranchAccessService {
  ...
  async ensureAccess(
    ...
    branchId: string,
    allowedRoles: Role[],
  ): Promise<PaymentsBranchAccessContext> {
    if (
      authUser.profileType !== LoginProfileType.STAFF ||
      !authUser.restaurantId
    ) {
      throw new ForbiddenException(
        'Solo un staff autenticado puede operar sobre el modulo payments.',
      );
    }
