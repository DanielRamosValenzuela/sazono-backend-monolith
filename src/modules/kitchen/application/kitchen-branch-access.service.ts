...
export type KitchenBranchAccessContext = {
  staffUserId: string;
  restaurantId: string;
  branchId: string;
  roles: Role[];
...
@Injectable()
export class KitchenBranchAccessService {
  ...
  async ensureAccess(
    ...
    branchId: string,
    allowedRoles: Role[],
  ): Promise<KitchenBranchAccessContext> {
    if (
      authUser.profileType !== LoginProfileType.STAFF ||
      !authUser.restaurantId
    ) {
      throw new ForbiddenException(
        'Solo un staff autenticado puede operar sobre el modulo kitchen.',
      );
    }
