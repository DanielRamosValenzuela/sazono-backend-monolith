...
export type FloorBranchAccessContext = {
  staffUserId: string;
  restaurantId: string;
  branchId: string;
  roles: Role[];
...
@Injectable()
export class FloorBranchAccessService {
  ...
  async ensureAccess(
    ...
    branchId: string,
    allowedRoles: Role[],
  ): Promise<FloorBranchAccessContext> {
    if (
      authUser.profileType !== LoginProfileType.STAFF ||
      !authUser.restaurantId
    ) {
      throw new ForbiddenException(
        'Solo un staff autenticado puede operar sobre el modulo floor.',
      );
    }
